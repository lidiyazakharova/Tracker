...
//    ]
    ...
    func addTracker(title: String, categoryTitle: String, schedule: [Weekday]) {
        let existsCategory = categories.first(where: { category in category.title == categoryTitle})

        if(existsCategory != nil) {
            categories.removeAll(where: { category in category.title == categoryTitle})
            categories.append(
                TrackerCategory(
                    title: categoryTitle,
                    trackers: (existsCategory?.trackers ?? []) + [Tracker(
                        id: UUID(),
                        title: title,
                        color: .blue,
                        emoji: "👕",
